...
public class ForgetPwdOneActivity extends BaseAppCompatActivity {
    private static final String LOG_TAG = "ForgetPwdOneActivity";
            ...
            case R.id.tv_next:
                ...
                if(isInputValid()){
                    if(!m_strPhoneSend.equals(m_strPhone)){
                        Utils.showToast(ForgetPwdOneActivity.this,"手机号已改变，请重新获取验证码");
                        return;
                    }
        ...
        HttpClient.get(ApiStores.user_send_yzm + urlDataString, new HttpCallback<ResponseBaseBean>() {
            ...
            public void OnSuccess(ResponseBaseBean response) {
                if(response.getResult()){
                    Intent it = new Intent(ForgetPwdOneActivity.this,ForgetPwdTwoActivity.class);
                    it.putExtra("strPhone",m_strPhone);
                    startActivity(it);
                    finish();
                }else{
                    Utils.showToast(ForgetPwdOneActivity.this,response.getMessage());
                }
        ...
        HttpClient.get(ApiStores.user_send_yzm ,ApiStores.user_send_yzm(userPhone), new HttpCallback<ResponseBaseBean>() {
            ...
            public void OnSuccess(ResponseBaseBean response) {
                ...
                if(response.getResult()){
                    ...
                    m_myCount = new SmsSendCounter(ForgetPwdOneActivity.this,m_tvSendVerifyCode, RESEND_VERIFY_CODE_SECOND * 1000, 1000);
                    m_myCount.start();
